...
The release bundle includes `Resources/ThemePacks`, a snapshot of the maintainer's managed Codex theme library. The package manifests and artwork are retained as supplied. Most packages identify `Codex Theme Studio` as author, while some packages have incomplete or legacy provenance metadata; Codex Studio does not make a blanket ownership claim over every included asset. Individual theme and artwork licenses remain the responsibility of their original creators, and package-level notices must be preserved when a package is reused.
