...
if [ "$IN_PLACE" = "false" ] && [ "$PROJECT_ROOT" != "$INSTALL_ROOT" ]; then
  ...
  [ "$RUNTIME_ONLY" != "true" ] || install_args+=(--runtime-only)
...
if [ "$RUNTIME_ONLY" = "true" ]; then
  ensure_state_root
  printf 'Codex Dream Skin runtime %s installed at %s.\n' "$SKIN_VERSION" "$PROJECT_ROOT"
  exit 0
fi
...
printf 'Bundled presets are ready in your theme library — pick one from the menu bar (Saved Themes) or switch-theme.\n'
